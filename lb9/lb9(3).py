...
def test_valid_pair_model(valid_pair_data):
    pair = PairModel(**valid_pair_data)
    assert pair

def test_invalid_pair_model(invalid_pair_data):
    with pytest.raises(ValueError):
        PairModel(**invalid_pair_data)

def test_valid_historical_data_model(valid_historical_data):
    historical_data = HistoricalData(**valid_historical_data)
    assert historical_data

def test_invalid_historical_data_model(invalid_historical_data):
    with pytest.raises(ValueError):
        HistoricalData(**invalid_historical_data)

def test_valid_pair_json(valid_pair_json):
    with open(valid_pair_json, 'r') as f:
        data = json.load(f)
    pair = PairModel(**data)
    assert pair

def test_invalid_pair_json(invalid_pair_json):
    with open(invalid_pair_json, 'r') as f:
        data = json.load(f)
    with pytest.raises(ValueError):
        PairModel(**data)

def test_valid_historical_json(valid_historical_json):
    with open(valid_historical_json, 'r') as f:
        data = json.load(f)
    historical_data = HistoricalData(**data)
    assert historical_data

def test_invalid_historical_json(invalid_historical_json):
    with open(invalid_historical_json, 'r') as f:
        data = json.load(f)
    with pytest.raises(ValueError):
        HistoricalData(**data)
